...
export const cardFormErrors = reactive({
  cardHolderName: computed(
    () =>
      cardFormWasTouched.cardHolderName && !cardFormValidities.cardHolderName
  ),
  cardCode: computed(
    () => cardFormWasTouched.cardCode && !cardFormValidities.cardCode
  ),
  expiration: computed(
    () => cardFormWasTouched.expiration && !cardFormValidities.expiration
  ),
...
export const cardFormValidities = reactive({
  cardHolderName: computed(() =>
    isValidFnameLname(cardFormData.cardHolderName)
  ),
  cardCode: computed(() => isValidCardCode(cardFormData.cardCode)),
  expiration: computed(() =>
    isValidCardExpirationDate(cardFormData.expiration)
  ),
...
export const isFormValid = computed(() =>
  deriveKeysValidity(cardFormValidities)
);
